...
# TO DO: check the following assumption:
# THEREFORE, THE DB FILE CANNOT CONTAIN TWO 
# LOCATIONS THAT SHARE OVERLAPS. IN ADDITION, THE TEMPLATE FILE MUST BE SORTED 
# FIRST BY CHROMOSOME THEN BY START LOCATION
# CHECK UCSC GENE ON REFCDS AND REFEXON
our $VERSION = 			'$Revision: 440 $';
our $LAST_CHANGED_DATE =	'$LastChangedDate: 2011-01-17 01:16:59 -0800 (Mon, 17 Jan 2011) $';

our ($verbose, $help, $man);
our ($queryfile, $dbfile);
our ($snp_flag, $score_threshold, $normscore_threshold, $overlap, $dbregion, $append, $condense_query, $mce_flag, $phastcons_flag, $evofold_flag, $dgv_flag, $isca_flag, $tfbs_flag,
...
# Set fields to print in the final output (1, print; 0, do not print)
# 1st flag is for $score
# 2nd flag is for $normscore
my %print_flag = (
                    mce       => [ 1, 1 ],
                    evofold   => [ 1, 0 ],
                    dgv       => [ 1, 1 ],
...
GetOptions('verbose|v'=>\$verbose, 'help|h'=>\$help, 'man|m'=>\$man, 'snp_flag'=>\$snp_flag, 'score_threshold=f'=>\$score_threshold, 
	...
	'mce_flag'=>\$mce_flag, 'phastcons_flag'=>\$phastcons_flag, 'evofold_flag'=>\$evofold_flag, 'dgv_flag'=> \$dgv_flag, 'isca_flag'=>\$isca_flag, 'tfbs_flag'=>\$tfbs_flag, 'wgrna_flag'=>\$wgrna_flag,
...
if ($minoverlap or $maxoverlap or $minquerydbratio or $maxquerydbratio or $mindbfrac or $maxdbfrac or $minqueryfrac or $maxqueryfrac) {
	#if ($mce_flag or $evofold_flag or $dgv_flag or $isca_flag or $tfbs_flag or $wgrna_flag or $segdup_flag or $anno_flag or $knowngene_flag or $refgene_flag or $refcds_flag or $refexon_flag or $phastcons_flag) {

    #FOR DB MULTIPLE SELECTION WHERE THERE ARE KNOWNGENE AND REFGENE IS NECESSARY TO SUPPRESS WARNINGS ABOUT THOSE OPTIONS

    #if ($knowngene_flag or $refgene_flag or $refcds_flag or $refexon_flag or $phastcons_flag) {
		#pod2usage ("Error in argument: the --minoverlap, --maxoverlap, --minquerydbratio, --maxquerydbratio, --mindbfrac, --maxdbfrac, --minqueryfrac, --maxqueryfrac arguments are NOT applicable to all DB format");
	#}
...
if ($score_threshold or $normscore_threshold) {
	if ($mce_flag or $evofold_flag or $dgv_flag or $isca_flag or $tfbs_flag or $wgrna_flag or $segdup_flag or $anno_flag) {
...
if ($expandmax or $expandleft or $expandright or $expanddb) {
	unless ($mce_flag or $evofold_flag or $dgv_flag or $isca_flag or $tfbs_flag or $wgrna_flag or $segdup_flag or $anno_flag or $knowngene_flag or $refgene_flag or $refcds_flag or $refexon_flag or $phastcons_flag) {
...
if ($mce_flag or $evofold_flag or $dgv_flag or $isca_flag or $tfbs_flag or $wgrna_flag or $segdup_flag or $anno_flag) {
	...
	if ($mce_flag) {
		$chr_loc_anno = scanUCSCRegion ($chr_loc, $dbfile, $count_query_bp, 'mce');
        printChrAnno($queryfile, $chr_loc_anno, 'mce');
	} elsif ($evofold_flag) {
		$chr_loc_anno = scanUCSCRegion ($chr_loc, $dbfile, $count_query_bp, 'evofold');
        printChrAnno($queryfile, $chr_loc_anno, 'evofold');
	} elsif ($dgv_flag) {
		$chr_loc_anno = scanUCSCRegion ($chr_loc, $dbfile, $count_query_bp, 'dgv');
        printChrAnno($queryfile, $chr_loc_anno, 'dgv');
	}elsif ($isca_flag) {
		$chr_loc_anno = scanUCSCRegion ($chr_loc, $dbfile, $count_query_bp, 'isca');
        printChrAnno($queryfile, $chr_loc_anno, 'isca');
	} elsif ($tfbs_flag) {
		$chr_loc_anno = scanUCSCRegion ($chr_loc, $dbfile, $count_query_bp, 'tfbs');
        printChrAnno($queryfile, $chr_loc_anno, 'tfbs');
	} elsif ($wgrna_flag) {
		$chr_loc_anno = scanUCSCRegion ($chr_loc, $dbfile, $count_query_bp, 'wgrna');
        printChrAnno($queryfile, $chr_loc_anno, 'wgrna');
	} elsif ($segdup_flag) {
		$chr_loc_anno = scanUCSCRegion ($chr_loc, $dbfile, $count_query_bp, 'segdup');
        printChrAnno($queryfile, $chr_loc_anno, 'segdup');
	} elsif ($anno_flag) {
		$chr_loc_anno = scanUCSCRegion ($chr_loc, $dbfile, $count_query_bp, 'anno');
        printChrAnno($queryfile, $chr_loc_anno, 'anno');
	}
...
} elsif ($phastcons_flag) {							#special treatment for phastcons file
	my ($chr_loc, $count_query_bp) = readChrLoc ($queryfile);
	$count_query_bp or print STDERR "WARNING: there is NOTHING in query to scan\n" and exit(0);	#there is no query regions in the query-location-file
...
sub printChrAnno{
    my ($queryfile, $chr_loc_anno, $db_type) = @_;
    my ($counter)=0;
	...
	#<QUERY> if <QUERY> !~ m/^(chr)?(\w+)\t(\d+)(\t(\d+))?(.*)/;
	while (<QUERY>) {
		/\S/ or next;												#skip empty lines
		s/[\r\n]+$//;												#get rid of newline characters
		next if $_ !~ m/^(chr)?(\w+)\t(\d+)(\t(\d+))?(.*)/ and $counter==0;
        ...
        #print Dumper $chr_loc_anno;

        print "chr$chr\t$loc_start\t$loc_end$other_info";
        if ($append){
            my @score     = defined $chr_loc_anno->{$chr}{$loc_start}{$loc_end}{"score"} ? @{$chr_loc_anno->{$chr}{$loc_start}{$loc_end}{"score"}} : ("NOT FOUND");
            my @normscore = defined $chr_loc_anno->{$chr}{$loc_start}{$loc_end}{"normscore"} ? @{$chr_loc_anno->{$chr}{$loc_start}{$loc_end}{"normscore"}} : ("NOT FOUND");

            # Extract the unique values from @score and @normscore, and return the values for both arrays in the same order
            my (%seen_score, %seen_normscore);
            my @ind_score = grep !$seen_score{ $score[$_] }++, 0..$#score;
            my @ind_normscore = grep !$seen_normscore{ $normscore[$_] }++, 0..$#normscore;
            # Now merge indexes and remove duplicates
            my @ind_merged = keys %{{map {($_ => 1)} (@ind_score, @ind_normscore)}};
            my @unique_score     = @score[ @ind_merged ];
            my @unique_normscore = @normscore[ @ind_merged ];
            #print "\t", join(',', @unique_score), "\t", join(',', @unique_normscore);
            print "\t", join(';', @unique_score) if $print_flag{$db_type}[0];
            print "\t", join(';', @unique_normscore) if $print_flag{$db_type}[1];
            #print "\t", join(',', @unique_score), "\t", join(',', @unique_normscore);
        }
        print "\n";
		$counter++;
...
# Updated by PU on Dec 2014: This is a modified version of the original subroutine: returns a hash with annotations
sub scanUCSCRegion {
    ...
    my %chr_loc_anno;                                       #contains all the overlapping score and normscore
	...
	while (<DB>) {
		...
		if ($dbtype eq 'mce') {
			@record == 6 or confess "Error: invalid record in dbfile $dbfile (expecting 10 fields in mce file): <$_>";
			...
			@record == 5 or confess "Error: invalid record in dbfile $dbfile (expecting 5 fields in dgv file): <$_>";
			($chr, $start, $end, $score, $normscore) = ($record[0], $record[1], $record[2], $record[3], $record[4]);
		} elsif ($dbtype eq 'isca') {
			@record == 7 or confess "Error: invalid record in dbfile $dbfile (expecting 8 fields in isca file): <$_>";
			($chr, $start, $end, $score, $normscore) = ($record[0], $record[1], $record[2], $record[4], $record[5]);
		} elsif ($dbtype eq 'tfbs') {
			@record == 8 or confess "Error: invalid record in dbfile $dbfile (expecting 10 fields in tfbs file): <$_>";
			($chr, $start, $end, $score, $normscore) = ($record[1], $record[2], $record[3], $record[7], $record[5]);
		} elsif ($dbtype eq 'wgrna') {
			@record == 10 or confess "Error: invalid record in dbfile $dbfile (expecting 10 fields in wgrna file): <$_>";
			($chr, $start, $end, $score, $normscore) = ($record[1], $record[2], $record[3], $record[4], $record[5]);
		} elsif ($dbtype eq 'segdup') {
			@record == 30 or confess "Error; invalid record in dbfile $dbfile (expecting 30 fields in segdup file): <$_>";
			($chr, $start, $end, $score, $normscore) = @record[1, 2, 3, 4, 5];
		...
		if ($dbtype =~ m/^(mce|evofold|dgv|tfbs|wgrna|segdup|anno)$/) {
		...
		for my $i ($pointer .. @$qloc-1) {
			...
			if ($qend < $start) {					#query end is before the start position of this db region
				...
				if ($qstart >= $start) {			#query contained completely within db region
                    ...
                    # Do nothing for the following filters as query is contained within hit
                    # $minoverlap
                    # $minqueryfrac
                    # $maxqueryfrac
					if ($dbregion) {
                    ...
                    if($append){
                        push @{$chr_loc_anno{$chr}{$qstart}{$qend}{"score"}}, $score;
                        push @{$chr_loc_anno{$chr}{$qstart}{$qend}{"normscore"}}, $normscore;
                        #print "\t$score\t$normscore";
                    }
                    #print "\n";
				} else {					#query overlap but upstream of db region
					#db:       <------------------------->
				...
				if ($qstart >= $start) {			#query overlap but downstream of db region
					...
					#query: <------------------------------>
					$count_db_bp_found += ($end-$start+1);
                    # Do nothing for the following filters as hit is contained within query
                    # $minoverlap
                    # $mindbfrac
                    # $maxdbfrac
                    if (defined $maxoverlap) {
...
sub scanUCSCRegionBak {
	...
	while (<DB>) {
		...
		if ($dbtype eq 'mce') {
			...
			@record == 8 or confess "Error: invalid record in dbfile $dbfile (expecting 8 fields in isca file): <$_>";
			($chr, $start, $end, $score, $normscore) = ($record[0], $record[1], $record[2], $record[4], $record[6]);
		} elsif ($dbtype eq 'tfbs') {
			@record == 8 or confess "Error: invalid record in dbfile $dbfile (expecting 10 fields in tfbs file): <$_>";
			($chr, $start, $end, $score, $normscore) = ($record[1], $record[2], $record[3], $record[7], $record[5]);
		} elsif ($dbtype eq 'wgrna') {
...
=head1 SYNOPSIS
 ...
 Optional arguments:
        ...
        Flags specifying type of db-file
            ...
            --dgv_flag          dbfile is UCSC DgvMerged annotation file
